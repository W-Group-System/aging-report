    ...
    <table style="">
        ...
        <tbody>
            ...
            @php
                ...
                $vatable_unit_price_exp1 = 0;
                $vatable_unit_price_exp2 = 0;
                $vatable_amount = 0;
                $vatable_amount_exp1 = 0;
                $vatable_amount_exp2 = 0;
                $total_vatable_amount_sum = 0;
                $vat_inclusive = 0;
            @endphp
            ...
            @foreach ($details as $detail)
                @php
                    ...
                    // } else {
                        ...
                        $total_quantity_exp1 += $detail->Quantity;
                        $total_amount_exp1 += $detail->Linetotal;
                        $vatable_unit_price_exp1 = ($total_amount_exp1 / $total_quantity_exp1) * 0.12;
                        $vatable_amount_exp1 = $total_amount_exp1 * 0.12;
                    } elseif ($detail->U_Label_as == 'Ricogel 83985') {
                        $total_packing_exp2 += $detail->U_Bagsperlot;
                        $total_quantity_exp2 += $detail->Quantity;
                        $total_amount_exp2 += $detail->Linetotal;
                        $vatable_unit_price_exp2 = ($total_amount_exp2 / $total_quantity_exp2) * 0.12;
                        $vatable_amount_exp2 = $total_amount_exp2 * 0.12;
                    } else {
                        $total_packing += $detail->U_Bagsperlot;
                        $total_quantity += $detail->Quantity;
                        $total_amount += $detail->Linetotal; 
            ...
            <tr>
                ...
                <td></td>
                <td>Add 12% VAT</td>
                <td>{{ optional($details->first())->DocCur }} {{ number_format($vatable_unit_price,2) }}</td>
            ...
            <td></td>
                ...
                <td></td>
                <td>Add 12% VAT</td>
                <td>{{ optional($details->first())->DocCur }} {{ number_format($vatable_unit_price_exp1,2) }}</td>
                <td>{{ optional($details->first())->DocCur }} {{ number_format($vatable_amount_exp1,2) }}</td>
            @endif
            @if ($total_packing_exp2 > 0)
            <tr>
                <td style="font-weight: bold">Ricogel 83985</td>
                <td>{{ $total_packing_exp2 }} {{ $details->first()->U_packUOM }}</td>
                <td>
                    @if ($details->first()->U_Netweight)
                        {{ number_format($details->first()->U_Netweight, 2) }}
                        {{ $details->first()->U_printUOM }}
                ...
                <td>
                    {{ number_format($total_quantity_exp2, 2) }} {{ $details->first()->U_printUOM }}
                </td>
                <td>
                    {{ optional($details->first())->DocCur }}
                    {{ $total_quantity_exp2 > 0 ? number_format($total_amount_exp2 / $total_quantity_exp2, 4) : 0 }} /
                    {{ $details->first()->U_printUOM == 'lbs' ? 'lb' : 'kg' }}
                </td>
                <td>{{ optional($details->first())->DocCur }} {{ number_format($total_amount_exp2, 2) }}</td>
            </tr>
            <tr>
                <td>{{ $details->first()->U_SupplierCode }}</td>
            </tr>
            <tr>
                ...
                <td></td>
                <td>Add 12% VAT</td>
                <td>{{ optional($details->first())->DocCur }} {{ number_format($vatable_unit_price_exp2,2) }}</td>
                <td>{{ optional($details->first())->DocCur }} {{ number_format($vatable_amount_exp2,2) }}</td>
            </tr>
            @endif
            <tr>
...
<div class="product-total">
    @php
        ...
        $vat_inclusive = $total + $total_vatable_amount_sum;
    ...
    <div class="total-left-column" style="margin-top:4.5px">
        ...
        <div class="info-row" style="margin-top:3px;">
            <span class="info-name"></span>
            <span class="info-colon"></span>
            <span class="info-detail">{{ optional($details->first())->DocCur }} {{ number_format($total_vatable_amount_sum,2) }}</span>
        </div>
        <div class="info-row">
            <span class="info-name"></span>
            <span class="info-colon"></span>
